...
- [x] 记录滥用风险和后续加固方案。
- [x] 增加 `LOCAL_FEEDBACK_JSON` Secret 作为无 Supabase 时的手动反馈回灌。
- **状态：** in_progress
...
- [x] 加入简单评估指标：反馈量、喜欢率、来源/栏目统计、正负反馈主题。
- [ ] 邮件打开率需要外部追踪像素或邮件服务商统计，MVP 不内置。
- **状态：** in_progress

### 阶段 8：验证与上线
- [x] 本地或 GitHub Actions 手动运行一次真实 arXiv 完整流程。
...
### 阶段 9：GitHub Trending 仓库推荐
- [x] 每日 workflow 在 arXiv 之外抓取 GitHub Trending daily 仓库。
