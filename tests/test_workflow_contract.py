...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_daily_workflow_optionally_exports_liked_papers_archive(self):
        ...
        self.assertIn("Prepare liked papers archive checkout", workflow)
        self.assertIn('ARCHIVE_URL="https://github.com/${LIKED_PAPERS_REPO}.git"', workflow)
        self.assertIn("favorites-archive", workflow)
    ...
    def test_daily_workflow_initializes_empty_favorites_archive_repo(self):
        ...
        self.assertIn('ARCHIVE_URL="https://github.com/${LIKED_PAPERS_REPO}.git"', workflow)
        self.assertIn('ls-remote --exit-code --heads "$ARCHIVE_URL" main', workflow)
        self.assertIn('clone --depth 1 --branch main "$ARCHIVE_URL" favorites-archive', workflow)
        self.assertIn("archive_status=$?", workflow)
        self.assertIn('if [ "$archive_status" -ne 2 ]; then', workflow)
